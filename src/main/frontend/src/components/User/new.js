...
import {FiMessageCircle, FiXCircle, FiUserPlus, FiSearch, FiRefreshCw} from 'react-icons/fi';
...
function Friends() {
    ...
    // 컴포넌트 마운트 시 userId와 token 로드import React, { useState, useEffect } from 'react';
    import {Link} from 'react-router-dom';
    import axios from 'axios';
    import styles from '../../assets/styles/Friends.module.css';
    ...
    function Friends() {
        ...
        const fetchFriendsData = async () => {
            ...
            try {
                ...
                setMessage(error.response?.data || '친구 데이터를 불러오는 데 실패했습니다.');
                setIsError(true);
            }
        };
        ...
        const handleAddFriend = async (friendToAdd) => {
            ...
            try {
                ...
                setMessage(error.response?.data || '친구 요청에 실패했습니다.');
                setIsError(true);
            }
        };
        ...
        const handleDeleteFriend = async (friendToDelete) => {
            ...
            try {
                ...
                setMessage(error.response?.data || '친구 삭제에 실패했습니다.');
                setIsError(true);
            }
        };
        ...
        const handleAcceptRequest = async (request) => {
            ...
            try {
                ...
                setMessage(error.response?.data || '친구 요청 수락에 실패했습니다.');
                setIsError(true);
            }
        };
        ...
        const handleRejectRequest = async (request) => {
            ...
            try {
                ...
                setMessage(error.response?.data || '친구 요청 거절에 실패했습니다.');
                setIsError(true);
            }
        };
        ...
        const handleCancelRequest = async (request) => {
            ...
            try {
                ...
                setMessage(error.response?.data || '친구 요청 취소에 실패했습니다.');
                setIsError(true);
            }
        };
                        ...
                        <section className={styles.sectionCard}>
                            <h2 className={styles.sectionTitle}>
                                ...
                                <button
                                    ...
                                    onClick={handleRefreshRecommendedFriends}
                                    title="추천 친구 새로고침"
                                >
                                    <FiRefreshCw/>
                                </button>
    ...
    useEffect(() => {
        const storedUserId = localStorage.getItem("USER_ID");
        const storedToken = localStorage.getItem("ACCESS_TOKEN"); // 토큰 가져오기

        if (storedUserId && storedToken) {
            setUserId(Number(storedUserId));
            setToken(storedToken); // 토큰 저장
        } else {
            setMessage("로그인이 필요합니다.");
            setIsError(true);
        }
    ...
    // 친구 목록 및 추천 친구 목록을 불러오는 함수
    const fetchFriendsData = async () => {
        if (!userId || !token) return; // userId 또는 token이 없으면 실행하지 않음
    ...
    // userId 또는 token이 설정되거나 변경될 때마다 친구 데이터를 불러옴
    useEffect(() => {
        if (userId && token) { // userId와 token 모두 있을 때만 fetch
            fetchFriendsData();
        }
    ...
    // 추천 친구 목록 새로고침 핸들러 (백엔드 API 호출)
    const handleRefreshRecommendedFriends = () => {
        fetchFriendsData(); // 추천 친구 목록도 다시 불러옴
        setMessage('추천 친구 목록을 새로고침했습니다.');
        setIsError(false);
    };

    // 내 친구 목록 필터링 (프론트엔드에서 검색)
    const filteredMyFriends = myFriends.filter(friend =>
        friend.nickname.toLowerCase().includes(searchTerm.toLowerCase())
    );

    // 추천 친구 목록 필터링 (프론트엔드에서 검색)
    const filteredRecommendedFriends = recommendedFriends.filter(recFriend =>
        recFriend.nickname.toLowerCase().includes(searchTerm.toLowerCase())
    );
